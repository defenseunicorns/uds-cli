...
4. **Move configuration into HCL**
   ...
   - Keep `architecture`, `log_level`, `tmp_dir`, and `UDSCacheDir` in the Next `options` block. Change `oci_concurrency` to `concurrency`. The legacy `--tmpdir` flag becomes `--tmp-dir`.
