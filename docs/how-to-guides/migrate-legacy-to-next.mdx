...
2. **Convert the bundle definition**
   ...
   package "podinfo" {
     ...
     signature_verification {
       public_key = <<-PACKAGE_PUBLIC_KEY
       replace-with-your-package-public-key
       PACKAGE_PUBLIC_KEY
     }
   }
   ```
   ...
   - Combine `repository` and `ref` into `source`. For a local Legacy `path`, resolve
     the Legacy archive path; use a Next package directory only when it has generated
     package metadata such as `checksums.txt`.
   - Rename `optionalComponents` to `optional_components`.
   - Move package signing settings into `signature_verification`. Legacy `publicKey`
     is inline key content, not a path; preserve it as an HCL string or heredoc. For
     keyless verification, use `signature_verification.keyless` with the same
         ...
         - name: unicorn-podinfo
           values:
             - sourcePath: ".podinfo.replicaCount"
               targetPath: ".podinfo.replicaCount"
             - sourcePath: ".podinfo.service.type"
               targetPath: ".podinfo.service.type"
   ```
...
4. **Move configuration into HCL**
   ...
   - Inspect package `variables`, chart `variables` mappings, and deploy-time
     `###ZARF_VAR_<NAME>###` usages. A matching Legacy value for a Zarf variable must
     be a top-level scalar, because it applies to every package; retain a nested copy
     only when a values-file template also needs it. Resolve the shared scope manually
     if another package requires a different value, a Zarf default, or a prompt for
     the same variable. Do not use `###ZARF_PKG_VAR_*###` for this check: it is a
     package-creation template prefix. Put other package-specific Helm settings in a
     values file.
   - Keep `architecture`, `log_level`, and `tmp_dir` in the Next `options` block. The legacy `--tmpdir` flag becomes `--tmp-dir`.
   - Do not automatically change `oci_concurrency` to `concurrency`. Legacy `oci_concurrency` limits remote OCI layer operations; Next `concurrency` controls concurrent package deployments within a dependency level and accepts values from `1` through `25`. Select a Next concurrency value only after deciding the intended deployment parallelism.
   ...
   Next applies built-in defaults, then adjacent `defaults.uds.hcl`, then
   `config.uds.hcl`, then explicit command-line flags. `config.uds.hcl` overrides
   matching values in `defaults.uds.hcl`.
...
5. **Test, create, and deploy the Next artifact**
   ...
   ```bash
   cd next
   CLI_FEATURES=NextMode=true uds bundle create . --config ./config.uds.hcl --signing-key ./keys/bundle-signing.key
   ```
