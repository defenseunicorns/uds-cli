...
Next mode separates source development deploys from artifact deploys.

## What you'll accomplish

Deploy from a definition or a local or OCI artifact. Select packages when a partial deployment is intentional.

## Prerequisites

- [UDS CLI installed](/cli/getting-started/installation/)
- A `bundle.uds.hcl` directory or a created `.tar.zst` artifact
- A matching bundle public key such as `./cosign.pub` for signed artifact examples
...
2. **Deploy an artifact**
   ...
   Do not use `--skip-signature-verification` for artifacts that must be trusted.
