...
4. **Resume an unchanged deployment**
   ...
   State or package-read failures stop the deployment after bundle pre-deploy hooks and before any package deploys. Resume does not support package pre-deploy hooks, and it does not detect values-file or deploy-time configuration-only changes. The same limitation applies to `bundle dev deploy`.
