...
1. **Deploy from the definition**
   ...
   # Override deploy-time variables from the command line
   CLI_FEATURES=NextMode=true uds bundle dev deploy ./my-bundle --set environment=development --set replicas=2
...
4. **Override variables**
   ...
   Override deploy-time variables with repeatable `--set key=value` flags when you do not need a separate config file:

   ```bash
   CLI_FEATURES=NextMode=true uds bundle deploy ./my-bundle.tar.zst \
     --public-key ./cosign.pub \
     --set environment=production \
     --set replicas=3
   ```

   Unquoted values are strings unless they are booleans, numbers, lists, or objects. Quote ambiguous values to keep them as strings, for example `--set 'replicas="3"'`. CLI variables override values from `config.uds.hcl` and `defaults.uds.hcl`.

5. **Resume an unchanged deployment**

   Re-run a deployment with `--resume` to skip packages Zarf recorded with the same package digest, namespace override, filtered component names, and successful component status:

   ```bash
   CLI_FEATURES=NextMode=true uds bundle deploy ./my-bundle.tar.zst --resume
   ```

   State or package-read failures stop the deployment before hooks or packages run. Resume does not support package pre-deploy hooks, including hooks added by a bundle pre-deploy hook. Resume also does not detect values-file or deploy-time configuration-only changes. The same limitation applies to `bundle dev deploy`.
