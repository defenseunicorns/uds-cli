...
4. **Resume an unchanged deployment**
   ...
   State or package-read failures stop the deployment before hooks or packages run. Resume does not detect values-file or deploy-time configuration-only changes. The same limitation applies to `bundle dev deploy`.
