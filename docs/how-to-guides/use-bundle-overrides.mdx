...
1. **Add static value overrides**
   ...
   Use `values` for static configuration. Template expressions in `values` remain supported for compatibility and bundle imports, but use `variables` for new configuration that is supplied at deploy time.
