...
You can deploy multiple instances of the same Zarf package in a bundle by giving each a unique `name` and deploying them into different namespaces using `overrides`. Each instance must have a unique `name` in the bundle and a `namespace` override on all Helm charts in that package to avoid conflicts.
...
<CardGrid>
  ...
  <LinkCard
    ...
    description="Stream real-time Pepr logs from a running cluster."
