...
Use `bundle inspect` to read metadata and `bundle verify` to check the bundle signature. Both commands accept local `.tar.zst` files and OCI references. Neither accepts a source directory or standalone `bundle.uds.hcl` file.

## What you'll accomplish

Inspect metadata, verify a signature with a public key or keyless constraints, and emit JSON or YAML for automation.

## Prerequisites

- [UDS CLI installed](/cli/getting-started/installation/)
- A local `.tar.zst` bundle artifact or an OCI bundle reference
- The public key or keyless certificate constraints for a signed artifact
...
1. **Inspect an artifact**
   ...
   ```
    ...
    Inspect displays bundle metadata, package sources, and dependencies. Its package status reflects checks from bundle creation. Inspect does not verify the bundle signature.
