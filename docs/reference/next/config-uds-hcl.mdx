...
| `UDSCacheDir` | string | `~/.uds-cache` | Sets the cache directory. An explicit `--uds-cache` flag overrides this value. |
...
During `bundle create`, Next writes selected Zarf image blobs to `<UDSCacheDir>/layers/<sha256-digest>`. It creates the cache's `layers` directory when the first image blob is written. Other package files are not cached, and Next does not yet read these cached layers.
