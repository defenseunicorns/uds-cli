...
For `inspect`, `verify`, `pull`, `deploy`, and `reconfigure`, CLI verification flags take precedence over this block. `--skip-signature-verification` is an explicit insecure bypass where the command supports it. `bundle create` and `bundle sign` use signing flags instead of this consumer trust policy.
