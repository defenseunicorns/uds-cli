...
`CLI_FEATURES` and `--features` configure UDS only when they appear before a
Zarf command. Arguments after `uds zarf`/`uds z` or `uds tools zarf`/`uds
tools z` are passed to Zarf unchanged, including Zarf's own `--features` and
