...
description: Reference for the UDS CLI Next alpha mode, why it exists, and what it currently supports.
sidebar:
  order: 4.2
---
...
Use `CLI_FEATURES=NextMode=true uds bundle dev remove` with a bundle definition while developing locally:

```bash
# Remove using bundle.uds.hcl from the current directory
CLI_FEATURES=NextMode=true uds bundle dev remove

# Remove using a bundle definition in a specific directory
CLI_FEATURES=NextMode=true uds bundle dev remove ./my-bundle

# Remove selected packages defined by the bundle
CLI_FEATURES=NextMode=true uds bundle dev remove ./my-bundle --packages uds_k3d_dev,init
```

Definition-based removal cannot verify bundle provenance or a bundle signature. The command prints a warning before removal. Use artifact-based removal for release and registry workflows.

Use `CLI_FEATURES=NextMode=true uds bundle remove` with a created local `.tar.zst` artifact or OCI reference:

```bash
# Remove using a signed local artifact
CLI_FEATURES=NextMode=true uds bundle remove ./uds-bundle-my-app-amd64-1.0.0.tar.zst --public-key ./cosign.pub

# Remove using a signed OCI artifact
...
CLI_FEATURES=NextMode=true uds bundle remove ./uds-bundle-my-app-amd64-1.0.0.tar.zst --public-key ./cosign.pub --prompt
```

Artifact removal verifies the bundle signature before removing packages. For an intentionally unsigned local alpha artifact, use `--skip-signature-verification`; this bypasses bundle-integrity verification and is not appropriate for signed release or registry artifacts.

Both commands remove packages in reverse dependency order. A partial removal that would leave another bundle package depending on a removed package is rejected unless you pass `--force`. Packages not currently deployed on the cluster are skipped with a warning.
...
| `--prompt` | `false` | Enable interactive confirmation before removal begins. |
| `--skip-signature-verification` | `false` | Artifact removal only. Skip bundle-signature verification for an intentionally unsigned or untrusted artifact. |
...
> [!NOTE]
> Only the `variables` block is supported in `defaults.uds.hcl`. The `options` block is not allowed and will produce an error. Use `config.uds.hcl` with `--config` to set options such as architecture or log level.
