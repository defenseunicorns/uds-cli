...
| `uds bundle reconfigure <bundle-artifact>` | Reconfigure bundle defaults and produce a new artifact. |
...
```bash
# Deploy a local artifact
CLI_FEATURES=NextMode=true uds bundle deploy uds-bundle-my-app-amd64-1.0.0.tar.zst

# Pull and deploy a tagged OCI artifact
CLI_FEATURES=NextMode=true uds bundle deploy oci://registry.example.com/my-org/my-app:1.0.0

# Deploy a digest-pinned OCI artifact
CLI_FEATURES=NextMode=true uds bundle deploy oci://registry.example.com/my-org/my-app@sha256:<digest>
```
...
```bash
# Create an unsigned derivative artifact from a local bundle.
...
# Reconfigure from an OCI bundle reference.
...
CLI_FEATURES=NextMode=true uds bundle pull oci://registry.example.com/my-org/my-app:1.0.0 -d ./downloads
```
...
All bundle commands support structured output through `--output` or `-o`:
...
CLI_FEATURES=NextMode=true uds bundle create -o yaml
```
