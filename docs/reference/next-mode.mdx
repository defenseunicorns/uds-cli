...
> [!NOTE]
> Use the task-focused guides for [creating a bundle](/cli/how-to-guides/next-create-a-bundle/), [deploying a bundle](/cli/how-to-guides/next-deploy-a-bundle/), [publishing and pulling artifacts](/cli/how-to-guides/next-publish-pull/), [reconfiguring a bundle](/cli/how-to-guides/next-reconfigure-a-bundle/), and [inspecting and verifying artifacts](/cli/how-to-guides/next-inspect-verify/). This page covers the format, design, and current capabilities.

## When to use Next mode

Use Next mode when your deliverable is a multi-package bundle artifact that should carry package content, dependencies, and environment defaults through registry-backed or offline workflows. Keep an existing `uds-bundle.yaml` workflow in Legacy mode while Next remains an alpha preview.
...
| Bundle artifact publishing and retrieval | Publish and pull artifacts with `uds bundle push` and `uds bundle pull` |
| Bundle reconfiguration | Replace artifact defaults with `uds bundle reconfigure` |
| Bundle inspection and verification | Inspect metadata and verify bundle signatures with `uds bundle inspect` and `uds bundle verify` |
...
# Deploy a signed OCI artifact
CLI_FEATURES=NextMode=true uds bundle deploy oci://registry.example.com/my-org/my-app:1.0.0 --public-key ./cosign.pub

# Deploy a signed digest-pinned OCI artifact
CLI_FEATURES=NextMode=true uds bundle deploy oci://registry.example.com/my-org/my-app@sha256:<digest> --public-key ./cosign.pub
```
...
By default, reconfigure verifies the source bundle signature before creating the derivative artifact. Use `--skip-signature-verification` only for a locally controlled unsigned alpha source. Do not use it to bypass verification for registry-backed artifacts. The new artifact must also be signed, or created with `--unsigned` for local alpha workflows.
...
# Pull a signed artifact to the current directory
CLI_FEATURES=NextMode=true uds bundle pull oci://registry.example.com/my-org/my-app:1.0.0 --public-key ./cosign.pub

# Pull a signed artifact to a specific existing directory
mkdir -p ./downloads
CLI_FEATURES=NextMode=true uds bundle pull oci://registry.example.com/my-org/my-app:1.0.0 -d ./downloads --public-key ./cosign.pub
```

By default, pull verifies the bundle signature. Do not use `--skip-signature-verification` for registry artifacts in a trusted workflow. Keep unsigned testing to locally created artifacts instead.
