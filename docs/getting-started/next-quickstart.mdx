...
1. **Create a bundle definition**
   ...
   Each package needs a signature verification policy. Use `verify = false` only for local alpha workflows. This bypasses package signature verification, so package origin and integrity are not established. See [Create a bundle in Next mode](/cli/how-to-guides/next-create-a-bundle/) for signed artifacts.
