...
func TestTaskRunner(t *testing.T) {
	...
	t.Run("run action", func(t *testing.T) {
		...
		stdOut, stdErr, err := e2e.UDS("run", "action", "--file", "testdata/legacy/tasks/tasks.yaml")
	...
	t.Run("run cmd-set-variable", func(t *testing.T) {
		...
		stdOut, stdErr, err := e2e.UDS("run", "cmd-set-variable", "--file", "testdata/legacy/tasks/tasks.yaml")
		require.NoError(t, err, stdOut, stdErr)
	...
	t.Run("run default task", func(t *testing.T) {
		...
		stdOut, stdErr, err := e2e.UDS("run", "--file", "testdata/legacy/tasks/tasks.yaml")
		require.NoError(t, err, stdOut, stdErr)
	...
	t.Run("run default task when undefined", func(t *testing.T) {
		...
		stdOut, stdErr, err := e2e.UDS("run", "--file", "testdata/legacy/tasks/tasks-no-default.yaml")
	...
	t.Run("run reference", func(t *testing.T) {
		...
		stdOut, stdErr, err := e2e.UDS("run", "reference", "--file", "testdata/legacy/tasks/tasks.yaml")
		require.NoError(t, err, stdOut, stdErr)
	...
	t.Run("run recursive", func(t *testing.T) {
		...
		stdOut, stdErr, err := e2e.UDS("run", "recursive", "--file", "testdata/legacy/tasks/tasks.yaml")
		require.Error(t, err, stdOut, stdErr)
	...
	t.Run("includes task loop", func(t *testing.T) {
		...
		stdOut, stdErr, err := e2e.UDS("run", "include-loop", "--set", setVar, "--file", "testdata/legacy/tasks/tasks.yaml")
		require.Error(t, err, stdOut, stdErr)
		require.Contains(t, stdErr, "task looping exceeded max configured task stack")
	})
	...
	t.Run("run cmd-set-variable with --set", func(t *testing.T) {
		...
		stdOut, stdErr, err := e2e.UDS("run", "cmd-set-variable", "--set", "REPLACE_ME=replacedWith--setvar", "--set", "UNICORNS=defense", "--file", "testdata/legacy/tasks/tasks.yaml")
		require.NoError(t, err, stdOut, stdErr)
	...
	t.Run("run remote-import", func(t *testing.T) {
		...
		setVar := fmt.Sprintf("GIT_REVISION=%s", gitRev)
		stdOut, stdErr, err := e2e.UDS("run", "remote-import", "--set", setVar, "--file", "testdata/legacy/tasks/tasks.yaml")
		require.NoError(t, err, stdOut, stdErr)
	...
	t.Run("run rerun-tasks", func(t *testing.T) {
		t.Parallel()
		stdOut, stdErr, err := e2e.UDS("run", "rerun-tasks", "--file", "testdata/legacy/tasks/tasks.yaml")
		require.NoError(t, err, stdOut, stdErr)
	})

	t.Run("run rerun-tasks-child", func(t *testing.T) {
		t.Parallel()
		stdOut, stdErr, err := e2e.UDS("run", "rerun-tasks-child", "--file", "testdata/legacy/tasks/tasks.yaml")
		require.NoError(t, err, stdOut, stdErr)
	})

	t.Run("run rerun-tasks-recursive", func(t *testing.T) {
		t.Parallel()
		stdOut, stdErr, err := e2e.UDS("run", "rerun-tasks-recursive", "--file", "testdata/legacy/tasks/tasks.yaml")
		require.Error(t, err, stdOut, stdErr)
		require.Contains(t, stdErr, "task looping exceeded max configured task stack")
	})
	...
	t.Run("test includes paths", func(t *testing.T) {
		t.Parallel()
		stdOut, stdErr, err := e2e.UDS("run", "foobar", "--file", "testdata/legacy/tasks/tasks.yaml")
		require.NoError(t, err, stdOut, stdErr)
	...
	t.Run("test action with multiple include tasks", func(t *testing.T) {
		...
		stdOut, stdErr, err := e2e.UDS("run", "more-foobar", "--set", setVar, "--file", "testdata/legacy/tasks/tasks.yaml")
		require.NoError(t, err, stdOut, stdErr)
		require.Contains(t, stdErr, "echo foo")
		require.Contains(t, stdErr, "echo bar")
		require.Contains(t, stdErr, "defenseunicorns is a pretty ok company")
	...
	t.Run("test action with multiple nested include tasks", func(t *testing.T) {
		...
		stdOut, stdErr, err := e2e.UDS("run", "extra-foobar", "--set", setVar, "--file", "testdata/legacy/tasks/tasks.yaml")
		require.NoError(t, err, stdOut, stdErr)
		require.Contains(t, stdErr, "echo foo")
		require.Contains(t, stdErr, "echo bar")
	...
	t.Run("test variable passing to included tasks", func(t *testing.T) {
		...
		stdOut, stdErr, err := e2e.UDS("run", "more-foo", "--set", "FOO_VAR=success", "--file", "testdata/legacy/tasks/tasks.yaml")
		require.NoError(t, err, stdOut, stdErr)
	...
	t.Run("run list tasks", func(t *testing.T) {
		...
		stdOut, stdErr, err := e2e.UDS("run", "--list", setVar, "--file", "testdata/legacy/tasks/tasks.yaml")
		require.NoError(t, err, stdOut, stdErr)
	...
	t.Run("test bad call to zarf tools wait-for", func(t *testing.T) {
		t.Parallel()
		_, stdErr, err := e2e.UDS("run", "wait-fail", "--file", "testdata/legacy/tasks/tasks.yaml")
		require.Error(t, err)
	...
	t.Run("test successful call to zarf tools wait-for", func(t *testing.T) {
		t.Parallel()
		_, stderr, err := e2e.UDS("run", "wait-success", "--file", "testdata/legacy/tasks/tasks.yaml")
		require.NoError(t, err)
		require.Contains(t, stderr, "succeeded")
	})

	t.Run("test task to load env vars using the envPath key", func(t *testing.T) {
		stdOut, stdErr, err := e2e.UDS("run", "env-from-file", "--file", "testdata/legacy/tasks/tasks.yaml")
		require.NoError(t, err, stdOut, stdErr)
	...
	t.Run("test that variables of type file and setting dir from a variable are processed correctly", func(t *testing.T) {
		t.Parallel()
		stdOut, stdErr, err := e2e.UDS("run", "file-and-dir", "--file", "testdata/legacy/tasks/tasks.yaml")
		require.NoError(t, err, stdOut, stdErr)
	...
	t.Run("test that env vars get used for variables that do not have a default set", func(t *testing.T) {
		...
		stdOut, stdErr, err := e2e.UDS("run", "echo-env-var", "--file", "testdata/legacy/tasks/tasks.yaml")
		require.NoError(t, err, stdOut, stdErr)
		require.NotContains(t, stdErr, "default")
	...
	t.Run("test that ARCHITECTURE env var is getting passed to runner", func(t *testing.T) {
		...
		stdOut, stdErr, err := e2e.UDS("run", "echo-architecture", "--file", "testdata/legacy/tasks/tasks.yaml")
		require.NoError(t, err, stdOut, stdErr)
