...
func TestBufferString(t *testing.T) {
	t.Run("renders packages in deployment order", func(t *testing.T) {
		b := &spec.UDSBundle{
			Metadata: spec.Metadata{Name: "example", Description: "test bundle", Version: "1.0.0"},
			Packages: []spec.Package{
				{Name: "api", Source: "oci://example/api:1", DependsOn: []spec.PackageRef{{Name: "database"}}},
				{Name: "database", Source: "oci://example/database:1", Namespace: "data", ValuesFiles: []string{"values.yaml"}},
			},
		}

		out := BufferString(t.Context(), iostreams.IOStreams{}, b).String()
		assert.Contains(t, out, "Name:        example")
		assert.Contains(t, out, "Description: test bundle")
		assert.Contains(t, out, "Version:     1.0.0")
		assert.Less(t, strings.Index(out, "database (deploy level: 0)"), strings.Index(out, "api (deploy level: 1)"))
		assert.Contains(t, out, "Namespace: data")
		assert.Contains(t, out, "DependsOn: database")
		assert.Contains(t, out, "Value Files: values.yaml")
	})

	t.Run("falls back to declaration order for an invalid graph", func(t *testing.T) {
		b := &spec.UDSBundle{
			Metadata: spec.Metadata{Name: "invalid"},
			Packages: []spec.Package{
				{Name: "api", Source: "oci://example/api:1", DependsOn: []spec.PackageRef{{Name: "missing"}}},
				{Name: "database", Source: "oci://example/database:1"},
			},
		}

		out := BufferString(t.Context(), iostreams.IOStreams{}, b).String()
		assert.Less(t, strings.Index(out, "  api\n"), strings.Index(out, "  database\n"))
		assert.NotContains(t, out, "deploy level")
	})
}
...
// testItem is a nested item rendered by printer tests.
type testItem struct {
	Label  string `json:"label"  yaml:"label"  text:"Label"`
	Source string `json:"source" yaml:"source" text:"Source"`
}

// testNested is a nested object rendered by printer tests.
type testNested struct {
	Key string `json:"key" yaml:"key" text:"Key"`
}

// sampleResult returns representative structured data for printer tests.
