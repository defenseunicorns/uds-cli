...
// PrepareBundleDir copies bundle fixture data into a temporary directory.
...
func createBundleFromTestData(t *testing.T, testDataRelPath, arch string) (string, *bundle.CreateResult, string, error) {
	t.Helper()
...
// CreateBundleFromTestDataCLI creates an artifact with the configured CLI binary.
...
// CreateBundleFromTestDataCLIWithBinary creates an artifact with a selected CLI binary.
...
// CreateBundleFromTestDataCobra creates an artifact through the Cobra command tree.
...
// CopyDir recursively copies a test fixture directory.
...
// CheckDockerRunning skips a test when Docker is unavailable.
...
// DeleteK3dCluster removes a test cluster if it exists.
