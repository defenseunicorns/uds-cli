...
// copyRequiredBlobsFromLayout copies manifests and their referenced content into a layout.
func copyRequiredBlobsFromLayout(dstBlobDir, srcBlobDir string, manifests []OciManifest) error {
	for _, m := range manifests {
		...
		manifestPath := filepath.Join(srcBlobDir, md.Encoded())
		...
		if err := writeBlobBytesIfMissingAndVerify(dstBlobDir, md, manifestBytes); err != nil {
			return err
		}
...
// copyDescriptorBlobFromLayout copies and verifies the blob referenced by a descriptor.
func copyDescriptorBlobFromLayout(dstBlobDir, srcBlobDir string, d ociDescriptor) error {
	dd, err := parseDigest(d.Digest)
	if err != nil {
		return err
	}
	src := filepath.Join(srcBlobDir, dd.Encoded())
	st, err := os.Stat(src)
	if err != nil {
		return err
	}
	...
	return copyBlobFileIfMissingAndVerify(dstBlobDir, src, dd)
}
