...
- **Write tests that give confidence**: Unless there is a technical blocker, every new feature and bug fix should be tested in the project's automated test suite. Although many of our tests are E2E, unit and integration-style tests are also welcomed. Note that unit tests can live in a `*_test.go` file alongside the source code, and E2E tests live in `tests/legacy/e2e`
...
### Building the app
We use the standalone [Maru](https://github.com/defenseunicorns/maru-runner) task runner to build UDS CLI.

To build the app, check out the [tasks](tasks.yaml) with `maru run --list-all`, find the appropriate build target for your system, and run it from the root of the repo, for example `maru run build-cli-mac-apple`. This creates a binary in the `build` directory that is used by the E2E tests.
...
#### E2E Tests
E2E tests reside in the `tests/legacy/e2e` directory. They use bundles located in `testdata/legacy/bundles` which contain Zarf packages from `testdata/legacy/packages`. Feel free to add new bundles and packages where appropriate. It's encouraged to write comments/metadata in any new bundles or packages to explain what they are testing.
...
#### Running Tests
- **Unit Tests**: To run unit tests, run `maru run test:all` from the root of the repo.


- **E2E Tests**: Build UDS CLI locally before running the relevant `maru run test:*` task. The binary in the `build` directory drives these tests.
