...
Next parses generated values files with Go `text/template` whenever configuration
variables are present. Scan every static Legacy override scalar, list element, and
object value for literal `{{` and `}}`. Preserve each literal delimiter in generated
YAML by emitting `{{"{{"}}` and `{{"}}"}}`, respectively, so rendering produces the
...
If it is unclear whether a source delimiter is literal or intended for Next rendering,
or its escaping cannot preserve the YAML value shape, mark it **needs literal-template
review** instead of classifying the override as safely transcribed.
