...
`, "")
	...
	require.ErrorIs(t, err, ErrSignBundle)
...
func TestVerify_RejectsDuplicateSignatureEvidence(t *testing.T) {
	...
	require.ErrorIs(t, err, ErrVerifyBundle)
...
func TestPull_ReportsUnsignedOCIBundle(t *testing.T) {
...
`, "")
	ref := strings.TrimPrefix(server.URL, "http://") + "/test/bundle:v1"
	config := newTestConfig()
	config.Options.PlainHTTP = true
	_, err := Push(t.Context(), source, ref, PushOptions{Config: config})
	require.NoError(t, err)

	pullOpts := PullOptions{
		Config:       config,
		Verification: VerificationPolicy{PublicKey: "unused"},
	}
	checks := []struct {
		name string
		run  func() error
		...
		{name: "pull", run: func() error {
			_, err := Pull(t.Context(), ref, t.TempDir(), pullOpts)
			return err
		}},
	...
	for _, check := range checks {
		t.Run(check.name, func(t *testing.T) {
			err := check.run()
			require.ErrorIs(t, err, ErrPullBundle)
			require.ErrorIs(t, err, ErrBundleNotSigned)
		})
	}
	...
	require.ErrorIs(t, err, ErrPullBundle)
	require.ErrorIs(t, err, ErrBundleNotSigned)
	require.Equal(t, 1, strings.Count(err.Error(), ErrBundleNotSigned.Error()))
}
